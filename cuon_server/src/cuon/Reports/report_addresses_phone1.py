...
class report_addresses_phone1:
    ...
    def getReportData(self, dicSearchfields, dicUser, oAddress, reportDefs):
        self.dicResults = {}
        self.fileName = reportDefs['DocumentPathListsAddresses'] + '/' +_('Addresslist1-') + `dicUser['Name']` + '.pdf' 
        reportDefs['pdfFile'] = os.path.normpath(self.fileName)
        
        dicResult =  oAddress.getPhonelist1( dicSearchfields, dicUser)
##        for i in dicResult:
##            for j in i.keys():
##                if isinstance(i[j],types.UnicodeType):
##                    try:
##                        i[j] = (i[j].decode('utf-8')).encode('utf-7')
##                    except:
##                        pass
##            
        ...
        # 4 dicReportData
        # 5 reportDefs
        return reportDefs['ReportPath'] + '/addresses_phonelist1.xml', dicUser, self.dicResults, self.dicReportData, reportDefs
